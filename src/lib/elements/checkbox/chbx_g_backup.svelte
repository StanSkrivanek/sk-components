<script lang="ts">
	...
	class Checklist {
		...
		checkForSelected(e: any) {
			...
			this.updateChecks(tar.name, isParent);
		}

		updateChecks(name: string, isParent = false) {
			if (name) {
				const parent = this.el.querySelector(`[name="${name}"][data-parent]`);
				const checkItems = this.el.querySelectorAll(`[name="${name}"]:not([data-parent])`);
				const checked = Array.from(checkItems).filter((el: any) => el.checked);
				console.log("🚀 ~ Checklist ~ updateChecks ~ checked:", checked)
...
<div class="checkbox-group-container">
	<div class="checkbox-group">
		<CheckboxSingle name={group} value={group} data-parent />
		{#each data as item}
			<CheckboxSingle name={group} value={item} data-child />
		{/each}
	</div>
	<!-- <div class="checkbox-group">
		<CheckboxSingle name="mobile" value="Apple Mobile" data-parent />
		{#each data as device}
